...
private:
    ...
    vp::IoAccSlave input_itf;
    vp::IoAccMaster refill_itf;
    ...
    vp::IoAccReqStatus handle_req(vp::IoAccReq *req);
    void check_state();
    ...
    cache_line_t *refill(int line_index, unsigned int addr, unsigned int tag, vp::IoAccReq *req, bool *pending);
    static void refill_response(vp::Block *__this, vp::IoAccReq *req);
    static void refill_retry(vp::Block *__this);
    cache_line_t *get_line(vp::IoAccReq *req, unsigned int *line_index, unsigned int *tag, unsigned int *line_offset);

    unsigned int stepLru();
    bool ioReq(vp::IoAccReq *req, int i);
...
void Cache::refill_response(vp::Block *__this, vp::IoAccReq *req)
{
    ...
    vp::IoAccReq *pending_req = (vp::IoAccReq *)_this->refill_pending_reqs.pop();
    ...
    _this->input_itf.resp(pending_req);
...
void Cache::fsm_handler(vp::Block *__this, vp::ClockEvent *event)
{
    ...
    // {
    //     vp::IoAccReq *req = (vp::IoAccReq *)_this->refill_pending_reqs.pop();
    ...
    //     if (_this->handle_req(req) == vp::vp::IoAccReq_OK)
    //     {
...
cache_line_t *Cache::refill(int line_index, unsigned int addr, unsigned int tag, vp::IoAccReq *req, bool *pending)
{
...
//     vp::IoAccReq *refill_req = &this->refill_req;
//     refill_req->init();
...
cache_line_t *Cache::get_line(vp::IoAccReq *req, unsigned int *line_index, unsigned int *tag, unsigned int *line_offset)
{
...
vp::IoAccReqStatus Cache::handle_req(vp::IoAccReq *req)
{
    // unsigned int line_index;
    // unsigned int tag;
    // uint64_t offset = req->get_addr();
    ...
    return vp::IoAccReqStatus::IO_ACC_REQ_GRANTED;
}

vp::IoAccReqStatus Cache::req(vp::Block *__this, vp::IoAccReq *req)
{
    // Cache *_this = (Cache *)__this;
...
Cache::Cache(vp::ComponentConf &config)
    ...
    input_itf(&Cache::req),
    refill_itf(&Cache::refill_retry, &Cache::refill_response)
{
