...
static inline void csr_decode(Iss *iss, iss_insn_t *insn, iss_reg_t pc)
{
    ...
    char* csr_name = (char*)malloc(std::strlen( iss_csr_name(iss, UIM_GET(0)).c_str() ) +1 );
    std::strcpy(csr_name, iss_csr_name(iss, UIM_GET(0)).c_str());
    insn->args[2].flags = (iss_decoder_arg_flag_e)(insn->args[2].flags | ISS_DECODER_ARG_FLAG_DUMP_NAME);
    insn->args[2].name = csr_name;
    insn->args[2].is_name_allocated = true;
#endif
}
