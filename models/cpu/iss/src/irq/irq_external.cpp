...
{
  this->iss.trace.msg("Setting vector table (addr: 0x%x)\n", base);
  for (int i=0; i<32; i++)
  {
...
{
  this->iss.trace.msg(vp::trace::LEVEL_TRACE, "%s %d\n", __FILE__, __LINE__);

  this->iss.trace.msg("Interrupting pending elw\n");
...
void Irq::irq_req_sync(void *__this, int irq)
{
  ...
  _this->iss.trace.msg(vp::trace::LEVEL_TRACE, "Received IRQ (irq: %d)\n", irq);
...
int Irq::check()
{
  ...
  else
  {
    ...
    {
      this->iss.trace.msg(vp::trace::LEVEL_TRACE, "Handling IRQ (irq: %d)\n", req_irq);
