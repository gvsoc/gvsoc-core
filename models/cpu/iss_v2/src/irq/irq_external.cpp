...
IrqExternal::IrqExternal(Iss &iss)
    : iss(iss)
{
    for (int i = 0; i < 32; i++)
    {
    ...
    irq_req_itf.set_sync_meth(&IrqExternal::irq_req_sync);
    this->iss.new_slave_port("irq_req", &irq_req_itf, (vp::Block *)this);
    this->iss.new_master_port("irq_ack", &irq_ack_itf);

    this->iss.new_reg("irq_enable", &this->irq_enable, false);

    this->iss.csr.mtvec.register_callback(std::bind(&IrqExternal::mtvec_access, this, std::placeholders::_1, std::placeholders::_2, std::placeholders::_3));
    this->iss.csr.stvec.register_callback(std::bind(&IrqExternal::stvec_access, this, std::placeholders::_1, std::placeholders::_2, std::placeholders::_3));

}

void IrqExternal::global_enable(int enable)
{
...
void IrqExternal::reset(bool active)
{
    if (active)
    {
        this->irq_req = -1;
...
bool IrqExternal::mtvec_access(iss_insn_t *insn, bool is_write, iss_reg_t &value)
{
    if (is_write)
    {
...
bool IrqExternal::stvec_access(iss_insn_t *insn, bool is_write, iss_reg_t &value)
{
    if (is_write)
    {
...
bool IrqExternal::mtvec_set(iss_addr_t base)
{
    base &= ~(3ULL);
...
bool IrqExternal::stvec_set(iss_addr_t base)
{
    base &= ~(3ULL);
...
void IrqExternal::elw_irq_unstall()
{
...
void IrqExternal::irq_req_sync(vp::Block *__this, int irq)
{
    IrqExternal *_this = (IrqExternal *)__this;
    ...
    if (irq != -1 && _this->iss.exec.wfi.get())
    {
        ...
        if (_this->iss.exec.stall_cycles > (_this->iss.clock.get_cycles() - _this->iss.exec.wfi_start))
        {
            _this->iss.exec.stall_cycles -= (_this->iss.clock.get_cycles() - _this->iss.exec.wfi_start);
        }
        else
        {
