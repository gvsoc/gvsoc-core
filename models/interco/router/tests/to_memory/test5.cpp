...
Test5::Test5(Testbench *top)
: TestCommon(top, "Test5"), fsm_event(this, Test5::entry)
{
    this->top = top;
}

void Test5::entry(vp::Block *__this, vp::ClockEvent *event)
{
    Test5 *_this = (Test5 *)__this;

    size_t size = 2048;
    size_t bw0 = _this->top->packet_size, bw1 = _this->top->bandwidth;
    int nb_inputs = 2;
    int nb_gens_per_input = 2;
    ...
    if (_this->step == 0)
    {
        printf("Test 5 entry (4 generators from 2 router inputs to 2 memories)\n");
    ...
    else
    {
        ...
        // Divide by 2 the cycles since the generators go to 2 memories
        expected /= 2;
