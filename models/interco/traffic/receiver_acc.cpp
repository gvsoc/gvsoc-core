...
private:
    ...
    std::queue<vp::IoAccReq *>stalled_reqs;
...
vp::IoAccReqStatus Receiver::req(vp::Block *__this, vp::IoAccReq *req)
{
...
void Receiver::pending_fsm_handler(vp::Block *__this, vp::ClockEvent *event)
{
    ...
    {
        vp::IoAccReq *req = (vp::IoAccReq *)_this->pending_reqs.pop();
...
void Receiver::ready_fsm_handler(vp::Block *__this, vp::ClockEvent *event)
{
    ...
    if (!_this->ready_reqs.empty())
    {
        ...
        vp::IoAccReq *req = (vp::IoAccReq *)_this->ready_reqs.pop();
        req->get_resp_port()->resp(req);
    }

    if (_this->stalled && _this->pending_reqs.size() < _this->max_pending_reqs)
    {
        vp::IoAccReq *stalled_req = _this->stalled_reqs.front();
