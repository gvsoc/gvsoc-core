    ...
    : vp::Component(config)
{
    ...
    this->bandwidth_0 = this->get_js_config()->get("bandwidth_0")->get_int();
    this->bandwidth_1 = this->get_js_config()->get("bandwidth_1")->get_int();
    ...
    printf("Starting tests (nb_router_in: %d, nb_router_out: %d, nb_gen_per_router_in: %d, "
        "shared_rw: %d, access_type: %s, bandwidth_0: %d, bandwidth_1: %d, packet_size: %d, input_fifo_size: %d)\n",
        this->nb_router_in, this->nb_router_out, this->nb_gen_per_router_in,
        this->shared_rw, this->access_type.c_str(), this->bandwidth_0, this->bandwidth_1, this->packet_size,
...
int64_t Testbench::get_expected(int size, int nb_inputs, int nb_gens_per_input, int nb_targets)
{
    ...
    size_t input_0_bw = std::min(this->bandwidth_0, this->packet_size) * nb_inputs;
    if (do_read && do_write && !this->shared_rw)
    {
        input_0_bw *= 2;
    }

    size_t router_0_bw = std::min(this->bandwidth_0, this->packet_size) * std::min(nb_inputs, 2);
    if (do_read && do_write && !this->shared_rw)
    {
        router_0_bw *= 2;
    }

    size_t input_1_bw = std::min(this->bandwidth_1, this->packet_size) * 2;
    if (do_read && do_write && !this->shared_rw)
    {
        input_1_bw *= 2;
    }

    size_t router_1_bw = std::min(this->bandwidth_1, this->packet_size) * std::min(nb_inputs, nb_targets);
    if (do_read && do_write && !this->shared_rw)
    {
        router_1_bw *= 2;
    }

    size_t bw = std::min(std::min(input_0_bw, input_1_bw), std::min(std::min(router_0_bw, router_1_bw), target_bw));
