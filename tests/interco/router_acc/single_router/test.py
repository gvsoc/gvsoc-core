...
class RouterTest(gvsoc.systree.Component):
    ...
    def __init__(self, parent, name, access_type, shared_rw, bandwidth, packet_size, input_fifo_size, use_memory,
...
class Testbench(gvsoc.systree.Component):
    ...
    def __init__(self, parent, name, access_type, shared_rw, synchronous, bandwidth, packet_size,
            input_fifo_size, use_memory, target_bw, accurate):
        super().__init__(parent, name)

        nb_router_in = 4
        nb_router_out = 4
        # Accurate mode does not allow multiple masters on same slave IO port
        nb_gen_per_router_in = 1 if accurate else 2
        ...
        for i in range(0, nb_router_in):
            for j in range(0, nb_gen_per_router_in):
                for k in range(0, 1 if accurate else 2):
...
class Chip(gvsoc.systree.Component):
    ...
    def __init__(self, parent, name=None):
        ...
        accuracy = IoAccuracy.ACCURATE
        super().__init__(parent, name, io_accuracy=accuracy)
        ...
        soc = Testbench(self, 'soc', access_type=access_type, shared_rw=shared_rw, synchronous=synchronous,
            bandwidth=bandwidth, packet_size=packet_size, input_fifo_size=input_fifo_size, use_memory=use_memory,
            target_bw=target_bw, accurate=accuracy==IoAccuracy.ACCURATE)
