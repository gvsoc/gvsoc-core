...
"""
Python generator for the cdc_fifo_gray Verilator-backed CDC bridge.

Wraps the generic ``utils.io_v2_cdc_rtl_bridge`` C++ component, pointing
``plugin_path`` at the Verilator plugin built by
``hw/cdc_rtl/cdc_fifo_gray/sim/Makefile``. The plugin wraps a
forward+reverse pair of common_cells `cdc_fifo_gray` instances and presents
...
_PLUGIN_BASENAME = 'vlt_cdc_fifo_gray_fast.so'
...
class IoV2CdcFifoGrayRtl(Component):
    """Wrapper for the cdc_fifo_gray Verilator plugin, backed by the generic
