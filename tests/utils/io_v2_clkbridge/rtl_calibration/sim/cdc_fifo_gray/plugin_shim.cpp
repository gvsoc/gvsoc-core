...
/*
 * Verilator plugin shim for the cdc_fifo_gray_pair design β€” two
 * cdc_fifo_gray instances (forward + reverse) wrapped under one DUT.
 *
 * Structurally identical to the cdc_2phase shim (same ports, same clock
 * driving, same CdcBridgeExchange ABI). The cdc_fifo_gray RTL provides a
 * spill-register-fronted output and gray-coded pointer synchronization
 * across LOG_DEPTH FIFO entries, which produces a different per-transfer
 * latency profile than cdc_2phase β€” the comparison harness exposes that.
 */

#include "verilator_plugin_helpers.hpp"
#include "Vcdc_fifo_gray_pair.h"
...
struct VlPlugin : gv_vlplugin::PluginCommon<Vcdc_fifo_gray_pair> {
...
static VlPlugin *vl_open(int argc, const char *const *argv) {
    ...
    p->dut.reset(new Vcdc_fifo_gray_pair(p->ctx.get()));
    ...
    p->setup_trace(gv_vlplugin::parse_plusargs(argc, argv),
                   /*ps_per_vcd_tick=*/1, "cdc_fifo_gray.fst");
    return p;
}
